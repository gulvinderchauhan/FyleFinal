...
async function displayRepositories(repositories, page, repositoriesPerPage,userData) {
  ...
  // console.log();
  repositories.slice(startIndex, endIndex).forEach(async repository => {
    const languageResponse = await fetch(`https://api.github.com/repos/${userData.login}/${repository.name}/languages`);
    ...
    repositoryDiv.innerHTML = `
      <div>
        ...
        <p>Language: ${Object.keys(language).join(', ') || 'No language available'}</p>
      </div>
